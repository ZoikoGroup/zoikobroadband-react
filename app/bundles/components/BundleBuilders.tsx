  ...
  return (
    <>
      <section
        ...
        className="w-full py-12 px-6 sm:px-10 dark:bg-gray-900"
      >
        <div className="max-w-7xl mx-auto text-center">
          <h2
            id="bundle-builder-heading"
            className="text-2xl sm:text-3xl font-bold text-[#10446C] dark:text-white"
          >
            Interactive Bundle Builder
          </h2>

          <p className="text-gray-500 dark:text-gray-300 text-base sm:text-lg leading-relaxed mt-3 mb-8 max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {bundleServices.map((item) => (
              <article
                key={item.label}
                className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 px-4 py-10 md:py-14 flex flex-col items-center gap-5 hover:shadow-lg transition"
              >
                <h3 className="text-xl lg:text-2xl font-bold text-[#10446C] dark:text-white text-center">
                  {item.label}
                </h3>

                <p className="text-sm lg:text-base text-[#10446C] dark:text-gray-300 text-center">
                  {item.description}
                </p>
                ...
                <div className="flex flex-wrap justify-center gap-2 mt-4">
                  {item.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="text-xs md:text-base bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full text-[#10446C] dark:text-gray-200"
                    >
                      {tag}
                    </span>
                  ))}
